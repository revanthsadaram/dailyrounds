...
check_alerts() {
    ...
    if (( $(echo "$CPU > 80" | bc -l) )); then
        echo "Warning: CPU usage exceeded 80%! Current: $CPU%"
    fi

    local MEM_PERCENT=$(awk "BEGIN {printf \"%.0f\", ($USED_MEM/$TOTAL_MEM)*100}")
    if (( MEM_PERCENT > 75 )); then
        echo "Warning: Memory usage exceeded 75%! Current: $MEM_PERCENT%"
    fi
...
while true; do
    ...
    # Extract disk_info from system_data
    DISK_INFO=$(echo "$SYSTEM_DATA" | grep -oP '[^ ]+,[^ ]+,[^ ]+,[^ ]+%')

    TOP_PROCESSES=$(echo "$SYSTEM_DATA" | grep -oP '\d+,[^\s]+,\d+\.\d+' | tr '\n' ' ')

    # Check alerts
    check_alerts "$CPU_USAGE" "$USED_MEM" "$TOTAL_MEM" "$DISK_INFO"
